...
//
// The video/audio encoder class for wrapping FFmpeg encoding API.
//
...
public:
	/**
	 * Constructor for initializing an object of FFEncoder.
	 *
...
public:
	...
	/**
	 * Get the video buffer which contains the encoded frame data.
	 *
	...
	/**
	 * Get the presentation time stamp of the video stream being encoded.
	 *
	...
	/**
	 * Get the size of the raw video frame (unit: in bytes/uint8_t).
	 *
	...
	/**
	 * Get the audio buffer which contains the encoded frame data.
	 *
	...
	/**
	 * Get the presentation time stamp of the audio stream being encoded.
	 *
	 * This is usually used for synchronizing the video and audio streams.
	 *
	 * @return A non-negative double representing the time stamp (in seconds).
	...
	/**
	 * Get the size of raw audio frame including all channels (unit: in bytes/uint8_t).
	 *
...
public:
	...
	/**
	 * Open the codec, output file and allocate the necessary internal structures.
	...
	/**
	 * Close the codec, output file and release the memories
	 *
	...
	/**
	 * Encode one video frame (just encode, won't write encoded data to output file).
	 *
	...
	/**
	 * Encode one audio frame (just encode, won't write encoded data to output file).
	 *
...
private:
	...
	/**
	 * Encode a video frame to the internal encoded data buffer
	 *
	...
	/**
	 * Convert the pixel format of the input image
	 *
	...
	/**
	 * Encode an audio frame to the internal encoded data buffer
	 *
