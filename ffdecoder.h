...
public:
    /**
     * Constructor for initializing an object of FFDecoder.
     */
...
public:
    /**
     * Get the video parameters which are available after open() is called
     */
    const FFVideoParam &getVideoParam() const;

    /**
     * Get the audio parameters which are available after open() is called
     */
    const FFAudioParam &getAudioParam() const;

    /**
     * Get the decoded video frame data.
     *
    ...
    /**
     * Get the size of the decoded video frame (unit: in bytes).
     *
     * If the return value of decodeFrame() is 0 (i.e. video stream),
    ...
    /**
     * Get the decoded audio frame data containing all channels.
     *
    ...
    /**
     * Get the size of the decoded audio frame containing all channels (unit: in bytes/uint8_t).
     * 
     * If the return value of decodeFrame() is 1 (i.e. audio stream),
    ...
    /**
     * Get the presentation time stamp of current packet being decoded
     *
    ...
    /**
     * Get the decoding time stamp of current packet being decoded
     *
...
public:
    /**
     * Open the input file, codecs, and allocate the necessary structures and memories.
     */
    int open();

    /**
     * Close the input file, codecs, and release the memories.
     *  Must be called after decoding process is finished.
     */
    void close();

    /**
     * Read a frame from the input file and decode it.
     *
...
private:
    ...
    // Decode a video frame from current packet, and store it in the video frame buffer
    int decodeVideoFrame(AVPacket &avpkt);

    // Decode an audio frame from current packet, and store it in the audio frame buffer
